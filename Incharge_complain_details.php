...
<head>
    ...
    if(isset($_POST['assign']))
    {
      if($_SERVER["REQUEST_METHOD"]=="POST")
      {
        ...
        $res1=mysqli_query($con,"SELECT p_id FROM police where p_name='$pname'");
        $q3=mysqli_fetch_assoc($res1);
        $pid=$q3['p_id'];
      
      
        $res=mysqli_query($con,"update complaint set inc_status='Assigned',pol_status='In Process',p_id='$pid' where c_id='$cid'");
...
<select class="form-control" name="police_name" style="margin-left:40%; width:250px;">
            <?php
                        $p_name=mysqli_query($con,"select p_name from police where location='$location'");
