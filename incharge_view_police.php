   ...
   $con=mysqli_connect("localhost","root","","crime_protal");
     ...
     if(isset($_POST['s2']))
    {
        if($_SERVER["REQUEST_METHOD"]=="POST")
        {
            ...
            $q1=mysqli_query($con,"delete from police where p_id='$pid'");
            $q3=mysqli_query($con,"update complaint set pol_status='null',inc_status='Unassigned',p_id='Null' where p_id='$pid'");
        }
    }
    
    
    $result=mysqli_query($con,"select p_id,p_name,spec,location from police where location='$location'");  
