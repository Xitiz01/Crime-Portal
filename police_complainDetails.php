...
<head>
   ...
   $con=mysqli_connect("localhost","root","","crime_protal");
    ...
    if(isset($_POST['status'])){
        ...
        {
            $upd=$_POST['update'];
            $qu1=mysqli_query($con,"insert into update_case(c_id,case_update) values('$cid','$upd')");
        }
    }
    ...
    if(isset($_POST['close'])){
        if($_SERVER["REQUEST_METHOD"]=="POST")
        {
            ...
            $qu2=mysqli_query($con,"insert into update_case(c_id,case_update) values('$cid','$up')");
            $q2=mysqli_query($con,"update complaint set pol_status='ChargeSheet Filed' where c_id='$cid'");
           
        }
    }
     $res2=mysqli_query($con,"select d_o_u,case_update from update_case where c_id='$cid'");
